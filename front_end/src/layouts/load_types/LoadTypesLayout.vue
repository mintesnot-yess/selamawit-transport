    ...
    <div class="flex-1 flex flex-col">
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                ...
                <form @input.prevent="searchLoadTypes(searchQuery)"
                    ...
                    <input v-model="searchQuery" @input="handleSearchInput"
                        ...
                        placeholder="Search load_types..." type="search" />
        ...
        <main class="p-4 md:p-6 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-surface-200">
                <div class="px-6 py-4 border-b border-surface-200">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h2 class="text-lg font-semibold text-surface-900">LoadTypes </h2>
                        ...
                        <button @click="openAddLoadTypesForm"
                ...
                <div class=" ">
                    <table class="min-w-full divide-y divide-surface-200">
                        ...
                        <tbody class="bg-white divide-y divide-surface-200">
                            ...
                            <template v-if="loadingLoadTypes">
                            ...
                            <template v-else>
                                <tr v-for="load_type in load_types" :key="load_type.id" class="hover:bg-surface-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-surface-900">
                                        {{ (pagination.from - 1) + (load_types.indexOf(load_type) + 1) }}
                                    </td>
                                ...
                                <tr v-if="load_types.length === 0 && !loadingVehicles">
                                    <td colspan="10"
                                        class="px-4 py-8 text-center text-surface-400 text-base font-medium">
                                        <i class="fas fa-dolly text-2xl mb-2 block"></i>
                                        No load types found
                                    </td>
                                </tr>
                            </template>
            ...
            <div id="side_form_container" class="side-form" :class="{ hidden: !isSideFormVisible }">
                <div
                    ...
                    <button aria-label="Close" @click="toggleAddLoadTypesForm"
                    ...
                    <div @click="toggleAddLoadTypesForm" id="side_form"
...
<script>
import loadTypeService from '@/services/load-types';
...
export default {
    ...
    data() {
        return {
            ...
            load_types: [],
            loading: false,
            loadingLoadTypes: false,
            error: null,
            success: null,
            editingLoadTypes: null,
            searchQuery: '',
            isSearching: false,
            isUpdating: false,
    ...
    methods: {
        async fetchLoadTypes(page = 1) {
            this.loadingLoadTypes = true;
            try {
                const response = await loadTypeService.getAll({
                    page: page,
                ...
                console.error('Error fetching load_types:', error);
                this.$toast.error("Failed to load load_types: " + error.message);
            } finally {
                this.loadingLoadTypes = false;
            }
        },
        ...
        async handleSubmitAdd() {
            ...
            try {
                const response = await loadTypeService.store(this.form);
                await this.fetchLoadTypes();
                ...
                this.error = error.response?.data?.message || error.message || "Failed to save load_type information";
            ...
            try {
                const response = await loadTypeService.update(this.form.id, this.form);
                await this.fetchLoadTypes();
                this.success = "Load Type updated successfully"
        ...
        async searchLoadTypes() {
            ...
            try {
                const response = await loadTypeService.search({
        ...
        async loadLoadTypes() {
            ...
            try {
                ...
                const response = await loadTypeService.getAll(params);

                this.load_types = response.data || [];
                this.updatePagination(response.meta || {});

            } catch (error) {
                console.error('Error loading load_types:', error);
                this.$toast.error('Failed to load load_types');
            } finally {
                this.loadingLoadTypes = false;
            }
        },
        ...
        async handleSubmit() {
            ...
            try {
                if (this.editingLoadTypes) {
                    await loadTypeService.update(this.editingLoadTypes.id, this.form);
                    this.$toast.success("Load Type updated successfully");
                } else {
                    await loadTypeService.store(this.form);
                    this.$toast.success("Load Type created successfully");
                }
        ...
        // load_type edit function
        editLoadTypes(load_type) {
