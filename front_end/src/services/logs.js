...
const bankService = {
    ...
    async store(bankData) {
        try {
            const response = await axios.post(`${API_BASE_URL}/logs`, bankData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
    ...
    async update(id, bankData) {
        try {
            const response = await axios.put(`${API_BASE_URL}/logs/${id}`, bankData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
                    'Content-Type': 'application/json',
    ...
    async delete(id) {
        try {
            const response = await axios.delete(`${API_BASE_URL}/logs/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
                }
            });
